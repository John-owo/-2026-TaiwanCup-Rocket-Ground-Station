...
中央監控區新增「GPS 即時位置」卡片，使用 Leaflet 顯示 OpenStreetMap 標準圖磚。地圖不需要 API Key 或計費帳戶。圖磚 URL 固定使用官方要求的 `https://tile.openstreetmap.org/{z}/{x}/{y}.png`，畫面右下角保留清楚可見的 `© OpenStreetMap contributors` attribution。
